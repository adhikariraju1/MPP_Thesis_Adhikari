...
df1 <- df[df$state.x != df$state.y, ] #This shows that many states don't correspond to each other. We need to list them

write.xlsx(df, "C:/RajuPC/MPP Final Thesis/MPP_Thesis_Adhikari/df.xlsx")

#TOTO: Two issues with merging bea_df and unemployment_df:
#1. bea_df has a county called: maui + kalawao whereas unemployment_df has one called maui. Since kalawao is a very small island, we see that even in the election data, it is not included. Hence we need to rename the county to simply show maui only.

#2. bea_df has most counties in Virginia (VA) combined with another county with a plus (+) sign, whereas they are separated in the unemployment_df. Election_df also has them separate. Not sure how to go about this issue. 

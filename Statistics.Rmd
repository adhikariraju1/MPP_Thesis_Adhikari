...
#TOTO: There are some counties that appear like this: fremont (includes yellowstone park). Some others appear like this: baltimore (independent). I want to remove the part inside the parenthesis so that only the county name appears. There is also a county called doña ana. Can we change all the ñ into n.
...
#TOTO: I have some counties that appear like this: c("san francisco", "/city"). I want to remove all the unncessary symbols so that it says only san francisco. There are also some that look like this: c("nantucket", "/town"). Here also we need to repeat the same procedure so it shows only nantucket.
...
df <- merge(unemployment_df, bea_df, by= c('county.fips', 'year'), all = TRUE) #Merging first two datasets

df <- df %>% 
  filter(state.x != "AK" & state.y != "AK") #filtering to remove AK from the dataset. After doing this, all the states matched between the two datasets. We check this using the unique function.

unique(df$state.x)
unique(df$state.y)

write.xlsx(df, "C:/RajuPC/MPP Final Thesis/MPP_Thesis_Adhikari/df.xlsx")

#But the county names might still be different and not aligned. 
df1 <- df[df$county.x != df$county.y, ]
write.xlsx(df1, "C:/RajuPC/MPP Final Thesis/MPP_Thesis_Adhikari/df1.xlsx")

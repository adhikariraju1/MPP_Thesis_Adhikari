...
packages <- c("bea.R", "acs", "magrittr", "httr", "tidyr", "blsAPI", "rjson", "readxl", "dplyr", "jsonlite",
              "stringr", "rJava", "xlsx", "qdap", "data.table")
...
names(election_df)[names(election_df)=="√Ø..year"] <- "year"
...
names(incumbency)[names(incumbency)=="Ô..year"] <- "year"
...
rural <- rural[c(1,4,5)]
names(rural)[names(rural)=="Ô..county.fips"] <- "county.fips"
